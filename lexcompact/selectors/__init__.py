    ...
    def choose(self, word_features: Any, candidates: Sequence[ReconstructionCandidate]) -> ReconstructionCandidate | None:
        for predicate in self.predicates:
            if feature_value(word_features, predicate.feature) == predicate.value:
                matching = [candidate for candidate in candidates if candidate.stage_id == predicate.stage_id]
                if matching:
                    return min(matching, key=candidate_value)
        return choose_by_stage(self.default_order, candidates)

    def as_dict(self):
        return {"selector_id": self.selector_id, "predicates": [[p.feature, p.value, p.stage_id] for p in self.predicates], "default_order": list(self.default_order)}
...
class HashedLogisticSelector:
    ...
    def _score(self, candidate: ReconstructionCandidate, features: Any) -> int:
        ...
        for name, value in feature_items:
            bucket = int.from_bytes(hashlib.blake2b(f"{name}={value}".encode(), digest_size=4, person=b"lxc-logit").digest(), "little") % self.bucket_count
    ...
    def choose(self, word_features: Any, candidates: Sequence[ReconstructionCandidate]) -> ReconstructionCandidate | None:
        return max(candidates, key=lambda candidate: (self._score(candidate, word_features), tuple(reversed(candidate_value(candidate))))) if candidates else None

    def as_dict(self):
        return {"selector_id": self.selector_id, "bucket_count": self.bucket_count, "weights": [[bucket, list(values)] for bucket, values in self.weights]}
...
def train_hashed_logistic(rows: Iterable[Mapping[str, Any]], *, bucket_count: int = 1024, max_bytes: int = 32768) -> HashedLogisticSelector:
    ...
    for row in rows:
        ...
        for name, value in (features.items() if isinstance(features, Mapping) else ()):
            bucket = int.from_bytes(hashlib.blake2b(f"{name}={value}".encode(), digest_size=4, person=b"lxc-logit").digest(), "little") % bucket_count
            counts.setdefault(bucket, Counter())[target] += 1
    weights = tuple((bucket, tuple(sorted(counter.items()))) for bucket, counter in sorted(counts.items()))
...
class RandomForestSelector:
    ...
    def choose(self, word_features: Any, candidates: Sequence[ReconstructionCandidate]) -> ReconstructionCandidate | None:
        votes = Counter(tree.choose(word_features, candidates).stage_id for tree in self.trees if tree.choose(word_features, candidates) is not None)
        if not votes:
            return None
        stage, _ = max(votes.items(), key=lambda item: (item[1], item[0]))
        return min((candidate for candidate in candidates if candidate.stage_id == stage), key=candidate_value)
...
class GradientBoostedTreeSelector:
    ...
    def choose(self, word_features: Any, candidates: Sequence[ReconstructionCandidate]) -> ReconstructionCandidate | None:
        ...
        return max(candidates, key=lambda candidate: (scores.get(candidate.stage_id, 0), tuple(reversed(candidate_value(candidate))))) if candidates else None
...
__all__ = ["CARTSelector", "ForestSelector", "GBDTSelector", "GradientBoostedTreeSelector", "HashedLogisticSelector", "PrioritySelector", "RandomForestSelector", "StaticPrioritySelector", "TreePredicate", "TreeSelector", "train_hashed_logistic"]
