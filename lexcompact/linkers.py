...
class LinkerTable:
    ...
    def candidates(self, word: str, literals: Mapping[str, object]) -> tuple[LinkerCandidate, ...]:
        ...
        for linker in self.linkers:
            ...
            while True:
                ...
                start = position + 1
        found.sort(key=lambda item: (-len(item.left), item.left, item.linker.spelling, item.right))
...
def german_linker_table(*, max_candidates: int = 32) -> LinkerTable:
    ...
    return LinkerTable(
        tuple(Linker(spelling) for spelling in ("s", "es", "n", "en", "e", "er", "ens")),
