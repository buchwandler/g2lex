...
class NeuralModel:
    ...
    def as_dict(self):
        return {
            ...
            "character_outputs": [[key, value] for key, value in self.character_outputs],
...
def train_neural(
    ...
    *,
    ...
    default = (
        Counter(output for values in counts.values() for output in values).most_common(1)[0][0]
        if counts
        else ""
    )
    ...
    def candidates(self, word: str, context: object = None) -> tuple[ReconstructionCandidate, ...]:
        return (
            ReconstructionCandidate(
                self.stage_id, (self.model.predict(word),), analysis_kind=self.model.architecture
            ),
        )
...
train_transformer = lambda pairs, **kwargs: train_neural(
    pairs, architecture="transformer", **kwargs
)
