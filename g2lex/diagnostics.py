...
def _rule_candidates(
    ...
    variants = tuple((literals or asset.literals)[component] for component in components)
...
def analyze_failures(
    ...
    *,
    ...
    result: dict[str, Any] = {
        "oracle_provenance": "offline-only; expected IPA is used for analysis and never runtime lookup",
        "baseline_word_count": len(source_words),
        "retained_groups": {key: len(value) for key, value in sorted(classifications.items())},
        "retained_words": {key: sorted(value) for key, value in sorted(classifications.items())},
        ...
        "failure_family_summary": {
            ...
            "boundary_local_recoverable_count": sum(
                count for family, count in family_counts.items() if family != "non-local mismatch"
            ),
        },
        "failure_details": mismatch_details,
        "boundary_patterns": sorted(
            family_patterns.values(),
            key=lambda value: (-int(value["support_count"]), str(value["edit_template"])),
        ),
    }
    return result


def linker_diagnostics(source: LexiconData, asset: ImplicitLexicon) -> dict[str, Any]:
