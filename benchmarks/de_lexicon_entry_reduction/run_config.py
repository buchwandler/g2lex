...
def _source_values(case: dict[str, Any], source_config: dict[str, Any]) -> tuple[str, Path | None, Path | None]:
    ...
    return source_id, Path(source_path) if source_path else None, Path(data_root) if data_root else None
...
def run_config(config_path: str | Path, *, output: str | Path | None = None) -> list[dict[str, object]]:
    ...
    for case in config.values["cases"]:
        ...
        if getattr(asset.membership, "backend_id", "") != requested_membership:
            raise RuntimeError(f"loaded membership backend differs from requested {requested_membership!r}")
        if getattr(asset.literals, "backend_id", "") != requested_literals:
            raise RuntimeError(f"loaded literal backend differs from requested {requested_literals!r}")
        ...
        (destination / "resolved-config.json").write_text(json.dumps(case, ensure_ascii=False, sort_keys=True, indent=2) + "\n", encoding="utf-8")
        (destination / "audit.json").write_text(json.dumps(audit_runtime_representation(asset), indent=2) + "\n", encoding="utf-8")
        (destination / "integration.json").write_text(json.dumps({"consumer": "mapping-compatible", "source_id": source_id, "status": "not-run", "reason": "KokoroG2P consumer package is not part of the fixture run", "phoneme_output_equality": None, "fallback_behavior_equality": None}, indent=2) + "\n", encoding="utf-8")
        ...
        (destination / "stage-coverage.json").write_text(json.dumps({"stages": summary.get("stage_coverage", {}), "candidate_available_count_by_stage": {stage: values.get("candidate_proposed", 0) for stage, values in summary.get("stage_coverage", {}).items()}}, indent=2) + "\n", encoding="utf-8")
        runtime_config = _merge(config.values["runtime"], case.get("runtime", {}))
        if bool(runtime_config.get("fresh_process", True)):
            runtime = benchmark(destination, source=source_id, data_root=data_root, path=source_path, sample_size=int(runtime_config["sample_size"]), repetitions=int(runtime_config["repetitions"]), seed=int(config.values["seed"]))
