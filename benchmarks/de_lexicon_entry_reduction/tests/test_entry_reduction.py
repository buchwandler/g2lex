...
def test_motivating_sum_rule_and_unknown_recombination() -> None:
    ...
    assert (
        result.metrics.baseline_word_count == 3
        and result.metrics.literal_word_count == 2
        and result.metrics.generated_word_count == 1
    )
...
def test_runtime_composer_has_no_oracle_and_keeps_ambiguous_word_literal() -> None:
    source = make_source(("A", "a"), ("C", "c"), ("AB", "x"), ("BC", "bc"), ("ABC", "abc"))
    result = build_implicit_lexicon(source)
    assert "ABC" in result.asset.literals
    assert result.asset.lookup_all("ABC") == ("abc",)
    assert result.asset.composer.derive(
        "ABC", literals=result.asset.literals, prefix_index=result.asset.literal_index
    ) == ("xc",)


def test_variant_order_is_exact() -> None:
    source = make_source(("A", "a1"), ("A", "a2"), ("B", "b1"), ("B", "b2"), ("AB", "a1b1"))
    result = build_implicit_lexicon(source)
...
def test_stress_rule_is_shared_and_oracle_free() -> None:
    ...
    compound = build_implicit_lexicon(source, rules=german_rules())
...
def test_membership_exact_and_deterministic() -> None:
    ...
    assert all(membership.contains(w) for w in words)
    assert not any(membership.contains(w) for w in adversarial_misses(words))
...
def test_serialization_round_trip_has_no_derived_table(tmp_path: Path) -> None:
    result = build_implicit_lexicon(make_source(("1", "a"), ("2", "b"), ("12", "ab")))
    data = dumps(result.asset)
    assert b"derived" not in data
    reloaded = loads(data)
    assert reloaded.lookup_all("12") == ("ab",)
    assert reloaded.is_known("12") and not reloaded.is_known("21")
    save(tmp_path / "candidate.lxc", result.asset)
    assert load(tmp_path / "candidate.lxc").lookup_all("12") == ("ab",)
    assert "derived.json" not in asset_members(result.asset)
...
def test_deterministic_build_and_runtime_audit() -> None:
    ...
    assert dumps(first.asset) == dumps(second.asset)
...
def test_optimizer_and_verification_metrics() -> None:
    ...
    assert verification["words_checked"] == 3 and verification["lossless"]
...
def test_selector_chooses_candidates_without_expected_ipa() -> None:
    ...
    selector = RuleSelector((SelectorPredicate("component_count", "2", "C0", 100),), "C1")
    selected = selector.select(
        features, (Candidate("C1", ("hˈaʊstˌyːɐ",)), Candidate("C0", ("hˈaʊstˈyːɐ",)))
    )
    assert selected and selected.rule_id == "C0"
...
def test_selector_round_trip_and_training_are_deterministic() -> None:
    ...
    first = train_selector(
        ({"features": features, "target_rule": "C0"} for _ in range(100)), min_support=10
    )
    ...
    assert all("ab" not in str(v) for v in first.as_dict().values())
...
def test_boundary_rules_are_shared_and_deterministic() -> None:
    ...
    assert BoundaryStressClassRule().applies("Haustür", ("Haus", "tür"), variants)
...
def test_linker_is_shared_and_membership_still_gates(tmp_path: Path) -> None:
    ...
    save(tmp_path / "candidate.lxc", result.asset)
    assert load(tmp_path / "candidate.lxc").lookup_all("Arbeitszeit") == ("az",)
