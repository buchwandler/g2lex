...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--components", default="2,3,4")
    p.add_argument("--rules", default="concat,compound")
    p.add_argument("--optimizer", default="greedy,utility")
    p.add_argument("--data-root", type=Path)
    p.add_argument("--path", type=Path)
    a = p.parse_args(argv)
    rows = []
    for components, rule, optimizer in product(
        (int(v) for v in a.components.split(",")), a.rules.split(","), a.optimizer.split(",")
    ):
