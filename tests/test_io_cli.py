...
def test_json_cli_roundtrip(tmp_path: Path):
    ...
    assert main(["reduce", str(source), str(asset), "--report", str(report)]) == 0
    assert main(["verify", str(source), str(asset)]) == 0
    loaded = load(asset)
    assert loaded.literal_word_count == 2
    assert loaded.generated_word_count == 1
    ...
    assert json.loads(report.read_text())["lossless"]
