...
from lexcompact.adapters import (
    parse_extended_tsv_bytes,
    parse_jsonl_bytes,
    parse_kokoro_json_bytes,
    parse_word_list_bytes,
)
from lexcompact.format_v5 import pack_typed
...
def test_typed_values_round_trip_and_selectors() -> None:
    ...
    lexicon = open_bytes(pack_typed(TypedLexiconData(entries), record_block_entries=2, key_block_entries=2))
    try:
...
def test_source_adapters_preserve_shapes() -> None:
    ...
    jsonl = parse_jsonl_bytes(b'{"word":"x","kind":"word"}\n{"word":"y","kind":"scalar","value":""}\n')
...
def test_aliases_layers_and_comparison() -> None:
    ...
    layered = LayeredLexicon([LexiconLayer("gold", {"a": None, "b": "gold"}, {}), LexiconLayer("silver", {"a": "silver", "c": "silver"}, {})])
    assert layered["a"] is None
    assert list(layered) == ["a", "b", "c"]
    result = compare({"a": "x"}, {"a": ("x",), "b": "y"})
    assert result.as_dict() == {"only_a": 0, "only_b": 1, "same": 0, "different": 1, "shape_different": 1}


def test_file_api_self_verification_and_export(tmp_path: Path) -> None:
    source = tmp_path / "source.json"
    source.write_text(json.dumps({"a": "x", "live": {"DEFAULT": "l", "VERB": "v"}}), encoding="utf-8")
    asset = tmp_path / "source.lxc"
    pack_file(source, asset, input_format="kokoro-json")
    exported = tmp_path / "restored.jsonl"
    from lexcompact import export_file, verify_file
...
def test_corrupt_v5_is_rejected() -> None:
