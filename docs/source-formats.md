...
| -------------- | ------------------------------------ | ------------------------------------------------------------------------------------ |
| `json-map`     | JSON object mapping words to values  | Typed values and deterministic key ordering are preserved.                           |
...
| `ipa-tsv`      | Word and slash-delimited IPA columns | One optional outer slash pair is removed; internal slashes are preserved.            |
...
For the plain two-column `tsv` adapter, every spelling is represented as a tuple
at the exact-value layer, including a spelling with only one physical row. For
example, one `Haus` row becomes `("pronunciation",)`, while repeated rows keep
their source order. Use `lookup_all()` or `lookup()` when a consumer needs a
pronunciation tuple or the first pronunciation instead of the exact typed value.
...
g2lex pack source.json dictionary.g2lex --format json-map
g2lex pack source-ipa.tsv dictionary.g2lex --format ipa-tsv
```
